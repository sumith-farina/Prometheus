...
def WindowsNetworkResourceUsageToCsv():
    ...
    for winIp in winIpLists:
        #送信トラフィック算出
        metrixName =  'sum(rate(' + sendTraffic + '{instance="' + winIp + ':9182"}[1d]))>0'
        response = requests.get(PROMETHEUS_URL + RANGE_QUERY_API,
        params={'query': metrixName, 'start': span, 'end': today_ts, 'step': RESOLUTION})
        #応答が返ってきているか確認
        status = response.json()['status']
...
def NodeCpuUsageToCsv():
    ...
    for nodeIp in nodeIpLists:
        ...
        #コア数取得
        metrixName =  '(1-sum(rate(' + nodeCpuSeconds + '{instance="' + nodeIp + ':9100",mode="idle"}[1d]))*100)'
        response = requests.get(PROMETHEUS_URL + RANGE_QUERY_API,
        params={'query': metrixName, 'start': span, 'end': today_ts, 'step': RESOLUTION})
        #応答が返ってきているか確認
        status = response.json()['status']
...
def NodeNetworkResourceUsageToCsv():
    ...
    #送信トラフィック算出
    for nodeIp in nodeIpLists:
        metrixName =  'sum(irate(' + sendTraffic + '{instance="' + nodeIp + ':9100",device!~"tap.*|veth.*|br.*|docker.*|virbr*|lo*"}[1d])*8)>0'
        response = requests.get(PROMETHEUS_URL + RANGE_QUERY_API,
        params={'query': metrixName, 'start': span, 'end': today_ts, 'step': RESOLUTION})
        #応答が返ってきているか確認
        status = response.json()['status']
...
#                    writeInfo = 'SendingTraffic(' + nicName + ')'
        ...
        #受信トラフィック算出
        metrixName =  'sum(irate(' + receiveTraffic + '{instance="' + nodeIp + ':9100",device!~"tap.*|veth.*|br.*|docker.*|virbr*|lo*"}[1d])*8) > 0'
        response = requests.get(PROMETHEUS_URL + RANGE_QUERY_API,
        params={'query': metrixName, 'start': span, 'end': today_ts, 'step': RESOLUTION})
        #応答が返ってきているか確認
        status = response.json()['status']
...
#      params={'query': metrixName, 'start': span, 'end': today_ts, 'step': RESOLUTION})
    ...
    response = requests.get(PROMETHEUS_URL + RANGE_QUERY_API,
    params={'query': metricsName, 'start': span, 'end': today_ts, 'step': RESOLUTION})
